...
class AgentDvrWeb {
  constructor(server, webSettings, adapter, instanceSettings, app) {
    ...
    adapter.on("objectChange", (id, obj) => {
      ...
      this._authHeader = native.user || native.pass ? "Basic " + Buffer.from((native.user || "") + ":" + (native.pass || "")).toString("base64") : null;
    });
    this._authHeader = native.user || native.pass ? "Basic " + Buffer.from((native.user || "") + ":" + (native.pass || "")).toString("base64") : null;
    app.get("/" + ROUTE + "/api/cameras", (req, res) => {
      ...
      Promise.all([
        ...
        adapter.getForeignStatesAsync(ns + ".cam_*.control.ptz.up"),
        // PTZ detection
    ...
    app.get("/" + ROUTE + "/api/config", (req, res) => {
      ...
      res.end(JSON.stringify({
        dashMaxRec: native.dashMaxRec || 200,
    ...
    app.get("/" + ROUTE + "/api/mjpeg", (req, res) => {
      const base = native.serverIp && native.port ? "http://" + native.serverIp + ":" + native.port : "";
      if (!base) {
        res.status(503).end("AgentDVR not configured");
        return;
      }
      const oid = String(req.query.oid || "");
      if (!oid || !/^\d+$/.test(oid)) {
        res.status(400).end("invalid oid");
        return;
      }
    ...
    app.get("/" + ROUTE + "/api/snap", (req, res) => {
      ...
      upReq.setTimeout(8e3, () => upReq.destroy());
      upReq.on("error", () => {
        if (!res.headersSent) res.status(502).end();
        else res.end();
      });
    ...
    app.get("/" + ROUTE + "/api/go2rtc/streams", (req, res) => {
      ...
      let target;
      try {
        target = new URL("/api/streams", native.go2rtcUrl);
      } catch (_) {
        res.setHeader("Content-Type", "application/json");
        res.end('{"streams":[]}');
        return;
    ...
    app.get("/" + ROUTE + "/api/ptz", (req, res) => {
      ...
      const DIRS = /* @__PURE__ */ new Set(["left", "upLeft", "up", "upRight", "right", "downRight", "down", "downLeft", "zoomIn", "zoomOut", "stop", "center"]);
      if (!camKey || !/^cam_[a-zA-Z0-9_-]+$/.test(camKey) || !DIRS.has(dir)) {
        res.status(400).json({ error: "invalid params" });
        return;
      }
      adapter.setForeignStateAsync(ns + "." + camKey + ".control.ptz." + dir, { val, ack: false }).then(() => {
        res.setHeader("Content-Type", "application/json");
        res.end('{"ok":true}');
      }).catch((err) => {
        adapter.log.warn("[agent-dvr] ptz error: " + err.message);
        res.status(500).json({ error: err.message });
      });
    });
    app.get("/" + ROUTE + "/api/record", (req, res) => {
      ...
      adapter.setForeignStateAsync(ns + "." + camKey + ".control." + cmd, { val: true, ack: false }).then(() => {
        res.setHeader("Content-Type", "application/json");
        res.end('{"ok":true}');
      }).catch((err) => {
        adapter.log.warn("[agent-dvr] record error: " + err.message);
        res.status(500).json({ error: err.message });
      });
    });
    app.get("/" + ROUTE + "/api/recordings", (req, res) => {
      ...
      adapter.getForeignStateAsync(ns + "." + camKey + ".events.json").then((state) => {
    ...
    app.get("/" + ROUTE + "/api/deleterecording", (req, res) => {
      const base = native.serverIp && native.port ? "http://" + native.serverIp + ":" + native.port : "";
      if (!base) {
        res.status(503).json({ error: "AgentDVR not configured" });
        return;
      }
      const oid = String(req.query.oid || "");
      const fn = String(req.query.fn || "");
      const camKey = String(req.query.camKey || "");
      if (!oid || !/^\d+$/.test(oid) || !fn) {
        res.status(400).json({ error: "invalid params" });
        return;
      }
      ...
      const upReq = mod.get({ hostname: target.hostname, port: target.port, path: target.pathname + target.search, headers }, (upstream) => {
        ...
        upstream.on("end", () => {
          ...
          const ok = !data.error && (!data.status || data.status === "ok");
          if (ok && camKey && /^cam_[a-zA-Z0-9_-]+$/.test(camKey)) {
            adapter.getForeignStateAsync(ns + "." + camKey + ".events.json").then((state) => {
              const raw = state && state.val ? String(state.val) : "[]";
              let items;
              try {
              ...
              if (Array.isArray(items)) {
                const updated = items.filter((item) => item.fn !== fn);
                if (updated.length !== items.length) {
                  return adapter.setForeignStateAsync(
                    ns + "." + camKey + ".events.json",
                    { val: JSON.stringify(updated), ack: true }
                  );
                }
              }
            }).catch(() => {
      ...
      upReq.setTimeout(1e4, () => upReq.destroy());
      upReq.on("error", () => {
        if (!res.headersSent) res.status(502).json({ error: "connection failed" });
      });
    });
    app.get("/" + ROUTE + "/webhook", (req, res) => {
      adapter.setForeignStateAsync(ns + ".system.control.refresh", { val: true, ack: false }).then(() => res.json({ ok: true })).catch((err) => res.status(500).json({ error: err.message }));
    });
    app.get("/" + ROUTE + "/api/systemstats", (req, res) => {
      ...
      upReq.setTimeout(5e3, () => upReq.destroy());
      upReq.on("error", () => {
        if (!res.headersSent) res.status(502).json({ error: "connection failed" });
      });
    });
    ...
    app.get("/" + ROUTE + "/api/media", (req, res) => {
      ...
      const upReq = mod.get({ hostname: target.hostname, port: target.port, path: target.pathname + target.search, headers }, (upstream) => {
        ...
        const ct = (upstream.headers["content-type"] || "video/mp4").replace(/video\/mkv\b/i, "video/x-matroska");
    ...
    server.on("upgrade", (req, socket, head) => {
      ...
      let target;
      try {
        target = new URL("/api/ws?src=" + encodeURIComponent(src), native.go2rtcUrl.replace(/\/+$/, ""));
      } catch (_) {
