...
function advCssVars(c?: WidgetColors): string {
	const cardBg = c?.cardBg || 'rgba(127,127,127,.15)';
	const tagBg = c?.tagBg || 'rgba(0,0,0,.55)';
	const tagText = c?.tagText || '#fff';
	const accent = c?.accent || '#8ab4f8';
	const modalBg = c?.modalBg || '#1c1c1e';
	const radius = c?.radius != null ? `${c.radius}px` : '8px';
...
function galleryCss(minCol: number, maxW: number, compact = false, colors?: WidgetColors): string {
	const gap = compact ? 6 : 12;
	return (
		`.advgrid{${advCssVars(colors)}display:grid;grid-template-columns:repeat(auto-fill,minmax(${minCol}px,1fr));gap:${gap}px;max-width:100%}` +
		...
		`.advimg{position:relative;display:block;border-radius:var(--adv-radius);overflow:hidden;background:var(--adv-card-bg)}` +
		`.advimg img{width:100%;height:auto;display:block}` +
		`.advimgfix img{height:100%;object-fit:cover}` +
		`.advtag{position:absolute;background:var(--adv-tag-bg);color:var(--adv-tag-color);font-size:.6rem;padding:2px 4px;border-radius:3px;line-height:1;max-width:85%;overflow:hidden;white-space:nowrap;text-overflow:ellipsis}` +
...
function galleryCssJs(minCol: number, compact = false, colors?: WidgetColors): string {
	const gap = compact ? 6 : 12;
	return (
		`.advroot{${advCssVars(colors)}font-family:inherit;color:inherit}` +
		...
		`.advtagbtn.on{background:var(--adv-accent);color:#fff;border-color:var(--adv-accent)}` +
		...
		`.advimgjs{position:relative;display:block;border-radius:var(--adv-radius);overflow:hidden;background:var(--adv-card-bg)}` +
		`.advimgjs img{width:100%;height:auto;display:block}` +
		`.advimgjs.advimgfix img{height:100%;object-fit:cover}` +
		`.advtagjs{position:absolute;background:var(--adv-tag-bg);color:var(--adv-tag-color);font-size:.6rem;padding:2px 4px;border-radius:3px;line-height:1;max-width:85%;overflow:hidden;white-space:nowrap;text-overflow:ellipsis}` +
...
class AgentDvr extends utils.Adapter {
	...
	private async onReady(): Promise<void> {
		...
		this.schedulePoll(pollSeconds * 1000);
	...
	private async fetchAvailableSources(): Promise<{ result: { source: string; name: string }[] }> {
		const deadline = new Promise<{ result: { source: string; name: string }[] }>(res =>
			this.setTimeout(() => res({ result: [{ source: '!', name: 'Timeout' }] }), 5000),
		);
	...
	private fetchGo2rtcStreams(overrideUrl?: string): Promise<{ streams: string[]; error?: string }> {
		...
		// go2rtc only accepts plain HTTP; force https:// → http://
			...
			httpReq = mod.get(target.toString(), res => {
				...
				res.on('end', () => {
					try {
						...
						resolve({
							streams,
							error: streams.length === 0 ? 'go2rtc reachable but no streams configured' : undefined,
						});
					} catch {
						resolve({
							streams: [],
				...
				res.on('error', (e: Error) => resolve({ streams: [], error: `Stream error: ${e.message}` }));
			});
			httpReq.on('error', (e: Error) => resolve({ streams: [], error: `Connection error: ${e.message}` }));
		});
	...
	private apiGet(path: string): Promise<ApiResult> {
		return new Promise(resolve => {
			const rawTimeout = this.config.httpTimeoutMs ?? 8000;
			const timeout = Math.max(1000, Math.min(30000, rawTimeout));
	...
	private async writeLeaf(id: string, val: unknown, unit?: string): Promise<void> {
		const t = typeof val;
	...
	private async writeStatusDps(d: Device, fid: string): Promise<void> {
		const data = d.raw.data as Record<string, unknown> | undefined;
	...
	private async buildDevice(d: Device): Promise<void> {
		...
		const CAM_ONLY = new Set(['snapshot', 'objectDetectOn', 'objectDetectOff']);
	...
	private async writeEventDps(d: Device, fid: string, events: Record<string, unknown>[]): Promise<void> {
		...
		await this.setStateAsync(jsonId, {
			val: JSON.stringify(
				events.slice(0, Math.min(5000, Math.max(1, this.config.dashMaxRec ?? 200))).map(ev => {
					const p = this.fmtEvent(ev, oid);
					return {
						fn: p.fn,
						date: p.date,
	...
	private schedulePoll(delayMs: number): void {
		...
		this.pollTimer = this.setTimeout(() => {
			void this.poll()
				.catch(e => this.log.warn(`Poll error: ${(e as Error).message}`))
				.finally(() => this.schedulePoll(delayMs));
		}, delayMs);
	}
