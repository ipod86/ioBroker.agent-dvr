...
class App extends GenericApp<GenericAppProps, AppState> {
    ...
    render(): React.JSX.Element {
        ...
        const handleChange = (key: string, value: any): void => {
            this.setState({
                native: { ...native, [key]: value } as any,
                changed: this.getIsChanged({ ...native, [key]: value } as any),
            });
        };
                ...
                <ThemeProvider theme={this.state.theme}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', bgcolor: 'background.default', color: 'text.primary' }}>
                        ...
                        <Box sx={{ flex: 1, overflowY: 'auto', p: 2, pb: 12 }}>
                            {tab === 0 && <ConnectionPanel native={native} onChange={handleChange} />}
                            {tab === 1 && <FeaturesPanel native={native} onChange={handleChange} />}
                            {tab === 2 && <WidgetPanel native={native} onChange={handleChange} />}
                            {tab === 3 && <DashboardPanel native={native} onChange={handleChange} socket={(this as any).socket} instance={(this as any).instance ?? 0} />}
                            {tab === 4 && <AdvancedPanel native={native} onChange={handleChange} />}
                        </Box>
                        {this.renderError()}
