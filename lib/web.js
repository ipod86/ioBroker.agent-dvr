...
class AgentDvrWeb {
    constructor(server, webSettings, adapter, instanceSettings, app) {
        ...
        app.get('/' + ROUTE + '/api/cameras', (req, res) => {
            ...
            ]).then(([nameStates, mjpegStates, snapStates, onlineStates, ptzStates]) => {
                const cameras = {};
        ...
        app.get('/' + ROUTE + '/api/config', (req, res) => {
            ...
            res.end(JSON.stringify({
                ...
                dashPtzVisible:      !!native.dashPtzVisible,
                dashRefreshSec:      native.dashRefreshSec      || 60,
                dashStreamReconnect: native.dashStreamReconnect !== false,
                dashColorBg:         native.dashColorBg         || '',
                dashColorSurface:    native.dashColorSurface    || '',
                dashColorAccent:     native.dashColorAccent     || '',
                dashColorText:       native.dashColorText       || '',
                dashColorBorder:     native.dashColorBorder     || '',
                dashColorOnline:     native.dashColorOnline     || '',
                dashColorOffline:    native.dashColorOffline    || '',
            }));
        });
