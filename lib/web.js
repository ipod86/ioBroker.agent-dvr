...
class AgentDvrWeb {
    constructor(server, webSettings, adapter, instanceSettings, app) {
        ...
        app.get('/' + ROUTE + '/api/recordings', (req, res) => {
            ...
            adapter.getForeignStateAsync(ns + '.' + camKey + '.events.json')
                .then(state => {
                    ...
                    // Always rewrite thumb/video URLs — recordings are typically viewed
                    // remotely, and the web extension caches native at startup so the
                    // enableStreamProxy flag cannot be read reliably at request time.
                    let items;
        ...
        // ── Recording thumbnail proxy (always active) ────────────────────────
        ...
        // ── Recording video proxy (always active) ────────────────────────────
