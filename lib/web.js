                ...
                .then(state => {
                    ...
                    if (Array.isArray(items)) {
                        items = items.map(rec => {
                            ...
                            // Migrate legacy streamFile.cgi URLs to video.mp4 (AgentDVR API change)
                            if (typeof out.video === 'string' && out.video.includes('streamFile.cgi')) {
                                out.video = out.video.replace('streamFile.cgi', 'video.mp4');
                            }
                            if (!native.enableStreamProxy) return out;
                            // Rewrite URLs to go through ioBroker proxy
