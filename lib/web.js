    ...
    constructor(server, webSettings, adapter, instanceSettings, app) {
        ...
        app.get('/' + ROUTE + '/api/cameras', (req, res) => {
            ...
            Promise.all([
                ...
                // Native camera aspect ratio (e.g. "1280/720") from adapter — TypeScript private,
                // but accessible at JS runtime; allows dashboard to correct FLV aspect ratio.
                const aspectMap = (adapter.camAspect && typeof adapter.camAspect === 'object')
                    ? adapter.camAspect : {};
                Object.entries(cameras).forEach(([, cam]) => {
                    cam.aspect = cam.oid ? (aspectMap[cam.oid] || '') : '';
                });
        ...
        // ── Debug: raw AgentDVR camera data ──────────────────────────────────
        app.get('/' + ROUTE + '/api/debug/cam', (req, res) => {
            const base = (native.ip && native.port) ? 'http://' + native.ip + ':' + native.port : '';
            if (!base) { res.status(503).end('AgentDVR not configured'); return; }
            const oid = String(req.query.oid || '');
            const url = oid && /^\d+$/.test(oid)
                ? base + '/command/getObject?oid=' + oid + '&ot=2'
                : base + '/command/getObjects';
            ...
            upReq.setTimeout(8000, () => upReq.destroy());
            upReq.on('error', () => { if (!res.headersSent) res.status(503).end('AgentDVR unreachable'); });
        });
